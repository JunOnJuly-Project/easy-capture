...
> **현재 상태: 구현 중 — 이미지 모드 완성 + 비디오 모드 코드 완료(GPU 실검증 일부 미완).**
> 이미지: 클릭→SAM2(CPU)→크롭→PNG/JPG + 종횡비·크기·업스케일(SwinIR). 비디오: 구간 추적→샷경계 재추적→크롭→GIF/MP4(슬로우모션은 계획 단계). SAM2 video·Grounding DINO 실추론은 Colab GPU 검증 경로(`poc/colab/`)로 진행 중.
...
- **core**: 순수 로직(`torch`/`PySide6`/`av`/`scenedetect` 비의존). 크롭(`crop`)·내보내기(`export`)·추적(`tracking`: gap_policy·rematch·shot_split·smooth)·추상 Protocol(`segmentation`: SegmentationBackend·VideoSegmentationBackend·DetectionBackend / `upscale`: UpscaleBackend / `source`: FrameSource).
...
| 영상 디코드 | PyAV · Pillow |
| 자동 검출·샷경계 재추적 (비디오, 코드 완료·GPU 실검증 미완) | Grounding DINO · PySceneDetect |

...
> 스키마 버전: v2
> 최종 업데이트: 2026-05-28 (비디오 샷경계 재추적 코드 완료 — Colab GPU 실추론 검증 대기)
...
```bash
.venv\Scripts\pytest -q     # 순수 로직 단위 테스트 (현재 280개)
```
...
### 현재 브랜치
`feature/video/shot-retrack` (비디오 샷경계 재추적 코드 완료 — Colab GPU 실추론 검증 대기)
...
3. **비디오 후속 슬라이스**: occlusion gap UI(CUT/FREEZE) → 수동 교정 UI(needs_correction → `core/correction`) → 오디오 동기(H4) → 업스케일 결합 → 타임라인 고도화. (샷경계 감지·재추적·Grounding DINO 재검출은 shot-retrack에서 완료)
4. (정리) `feature/poc-core`·`feature/app/scaffolding`·`feature/image/capture-slice`·`feature/image/crop-ux`·`feature/image/upscale`·`feature/video/tracking-slice`·`feature/video/shot-retrack` → main PR/머지.

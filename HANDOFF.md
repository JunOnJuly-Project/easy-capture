...
> 스키마 버전: v2
> 최종 업데이트: 2026-05-28 (슬로우모션 Story 1 — core 타임리맵 순수 로직 완료·main 머지, Story 2 진행 예정)
...
```bash
.venv\Scripts\pytest -q     # 순수 로직 단위 테스트 (현재 355개)
```
...
### 현재 브랜치
`main` (전 슬라이스 + 슬로우모션 Story 1 머지 — 355 테스트 통과). **새 슬라이스는 `main`에서 분기**(선형 누적 안티패턴 중단). 다음: Story 2(GIF per-frame duration) = `feature/export/gif-variable-duration`.
...
1. **🎬 슬로우모션(타임리맵) 구현** (진행 중, 계획서 `docs/plans/video-speed-remap-plan.md`):
   ...
   - ⏳ 다음: Story 2(GIF per-frame duration, `feature/export/gif-variable-duration`) → Story 3(MP4 복제) → Story 4(export 결합) → Story 5(UI 구간 테이블·미리보기 버튼) → Story 6(노트북). 각 Story `/develop --tdd`, main에서 분기.

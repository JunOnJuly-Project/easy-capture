...
- 🔬 **EdgeTAM(B) PoC 인프라 완료**(10fps 도전): `infra/edgetam_video_backend.py`(`EdgetamVideoBackend(Sam2VideoBackend)` — `_ensure_loaded`만 `EdgeTamVideoModel` override, SAM2 API 호환·fp16 계승, 5테스트)·노트북 셀1 `transformers>=5.10`·셀8.6(EdgeTAM vs SAM2-small fp16 비교 + box/negative 지원 확인). **다음(Colab T4 대기)**: 셀8.6 측정 → box/negative 지원·fps 향상·AC-01 100% 3개 통과 시 router 채택(새 ADR), 미달 시 SAM2 유지.

...
2. **(보강) 동적 패스트 상한:** UI/노트북에서 선택 가능한 최대 factor를 `min(4.0, 50 / base_fps)` 식으로 동적 산출(duration ≥ 20ms 보장: `(1000/base_fps)/factor ≥ 20` ⟹ `factor ≤ 1000/(base_fps×20) = 50/base_fps`). **fps 높을수록 상한 낮아짐**(예 120fps면 ~0.4x, 12fps면 4.0 클램프). MVP는 우선 (1) 클램프+경고로 안전을 보장하고, 동적 상한 노출은 UI 슬라이스(S5)에서 base_fps를 아는 시점에 적용한다. (구현 정답은 `50/base_fps` — `base_fps×0.02`는 역수 오기였음, Story 5에서 정정.)

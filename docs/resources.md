...
|---|---|---|---|
| 권장 | NVIDIA GPU 6GB+ (RTX 3060급) | 10~15 fps (추정) | 실사용 쾌적 |
| 가능 | NVIDIA GPU 4GB | 5~10 fps (Tiny, 추정) | 사용 가능 |
| **CPU (i7급, 14스레드)** | **≈ 0.10 fps (실측, PoC)** | **비실용적** — 6초 클립에 ~14분. 단발 이미지/초단편 보조용 |
| 업스케일 | GPU 권장 | CPU 시 분 단위 | CPU 경고 표시 |

> ⚠ **PoC 실측(2026-05-27, CPU 전용)**: SAM2.1-tiny 가 입력 해상도와 무관하게 내부 1024×1024 인코딩으로 **프레임당 ~10초**(0.10 fps)다. **GPU(CUDA)가 사실상 필수**이며, CPU 폴백은 강한 경고와 함께 단발/초단편으로 한정한다. 상세: [poc/REPORT.md](../poc/REPORT.md). GPU 수치는 CUDA 환경 벤치마크로 확정 예정.

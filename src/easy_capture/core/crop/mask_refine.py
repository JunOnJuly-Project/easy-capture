...
[deprecated] ADR 0014 갱신 → ADR 0015로 추적 후처리(_propagate_*)에서 철회.
  WHY 철회: 성능(720p 440ms/frame, 2.3→0.7fps)·효과(연결이 합쳐진 군무 밀착
  구간은 분리하지도 못함) 한계. 옆 멤버 분리는 negative point(label 0)가 대체한다.
  함수·테스트는 순수 회귀 가드로 백로그에 보존한다(재도입 가능성 대비).

배경(보존):
  멀티샷 군무에서 SAM2 마스크가 부정확·과대(대상 배 + 옆사람 팔)했다. box
  프롬프트(detect bbox→SAM2)로 1차 개선하되, 그래도 인접한 다른 사람의 작은
  파편이 마스크에 섞일 수 있다. 해결 시도: 마스크에서 가장 큰 연결성분(largest
